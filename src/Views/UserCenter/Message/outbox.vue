...
<div class="user-outbox">
    ...
    <div>
        <el-form ref="form" :model="form" label-width="80px" v-if="lan == 'zh'">
            ...
            <el-form-item :label="$t('tabel.Status')">
                <el-select v-model="form.Status" :placeholder="$t('text.input')+ $t('tabel.Status')+$t('text.here')">
        ...
        <el-table v-loading="loading" :data="tableData">
            ...
            <el-table-column :label="$t('tabel.Date')" width="">
                <template slot-scope="scope">
                    <div>{{scope.row.AddTime.split('T')[0]}}</div>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        seeData(item) {
            this.$router.push('/user/contactLetter?id=' + item.ID + '&type=from')
        },
        nextSearch() {
            this.$router.push('/user/outbox?title=' + (this.form.Title || '') + '&supplyName=' + (this.form.SupplyName || '') + '&isRead=' + (this.form.Status || ''))
        },
        handleCurrentChange(val) {
            this.$router.push('/user/outbox?title=' + (this.form.Title || '') + '&supplyName=' + (this.form.SupplyName || '') + '&isRead=' + (this.form.Status || '') + '&page=' + val)
        }
    },
    watch: {
        '$route'() {
