...
<script>
  import { getAllProducts } from "@/assets/api/api";
  export default {
    name: "viewAllOffers",
    data(){
      return{
    ...
    created() {
      getAllProducts({SupplyID:this.$route.query.id,page:0,size:10})
        .then(result=>{
    ...
    methods:{
      ...
      nextSearchView(item){
        this.$router.push('/MatchupExpo/search/view?id='+item.ID+'&type=product')
      },
    }
  }
</script>
